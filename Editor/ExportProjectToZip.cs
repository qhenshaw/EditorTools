...
namespace ExportProjectToZip
{
    ...
    public class ExportProjectToZip : MonoBehaviour
    {
        ...
        public static void ExportToZip()
        {
            ...
            { 
                GetFolderFullPath(".git"),
                GetFolderFullPath(".github"),
                GetFolderFullPath(".vs"),
                GetFolderFullPath(".vscode"),
                GetFolderFullPath("Builds"),
                GetFolderFullPath("builds"),
                GetFolderFullPath("Library"), 
                GetFolderFullPath("Logs"), 
                GetFolderFullPath("obj"), 
                GetFolderFullPath("Obj"), 
                GetFolderFullPath("Recordings"), 
                GetFolderFullPath("Temp"), 
                GetFolderFullPath("UserSettings") 
            };
            string[] topLevelFiles = Directory.GetFiles(projectPath, "*.*", SearchOption.TopDirectoryOnly);
            foreach (string file in topLevelFiles) { if (
                    Path.GetExtension(file) == ".sln" || 
                    Path.GetExtension(file) == ".csproj" ||
                    Path.GetExtension(file) == ".vsconfig" ||
                    Path.GetExtension(file) == ".log" ||
                    Path.GetExtension(file) == ".zip")
                { exceptionList.Add(file); } } //excludes .sln and .zip
